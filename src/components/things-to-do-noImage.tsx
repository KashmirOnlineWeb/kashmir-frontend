...
const ThingstodoNoImage: NextPage<ThingstodoNoImageProps> = ({ parseContentState, hasImage }) => {
    const { ref, inView } = useInView({
        rootMargin: '500px 0px 0px 0px',
    });
    const [fixedState, setFixedState] = useState(false);
        ...
        >
            <div className="flex flex-col w-full items-start justify-start">
                ...
                {parseContentState.repeater_content && (
                    <div
                        dangerouslySetInnerHTML={{ __html: parseContentState.repeater_content }}
                        className="relative w-[100%] floara-editor-ml-0 items-center text-base leading-[26px]"
                    />
                )}
            </div>
        </div>
...
export default ThingstodoNoImage;
